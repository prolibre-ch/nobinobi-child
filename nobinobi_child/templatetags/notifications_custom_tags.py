...
def dfu_live_notify_badge(context, badge_class='live_notify_badge'):
    ...
    html = "<span class='{badge_class}'>{unread}</span>".format(
        badge_class=badge_class, unread=user.notifications.filter(timestamp__lte=timezone.localtime()).count()
    )
    return format_html(html)
...
def dfu_register_notify_callbacks(badge_class='live_notify_badge',
    ...
    if api_name == 'list':
        api_url = reverse('nobinobi_child:dfu_live_all_notification_list')
    elif api_name == 'count':
        api_url = reverse('nobinobi_child:dfu_live_all_notification_count')
    else:
        return ""
    definitions = """
