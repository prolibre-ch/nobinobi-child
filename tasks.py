...
def coverage(c):
    ...
    """
    c.run("coverage run --source nobinobi_child runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/nobinobi-child.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ nobinobi_child")
...
def lint(c):
    ...
    """
    c.run("flake8 nobinobi-child tests")
...
def release(c, bumpsize=''):
    ...
    import nobinobi_child
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=nobinobi_child.__version__))
    c.run("git push --tags")
